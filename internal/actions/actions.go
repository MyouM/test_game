...
func (d *GameData) Move(cmnds []string) string {
	...
	if places.IsGlobalPlace(cmnds[1]) {
		newLocation = places.FromGlobToRoom(cmnds[1])
	} else {
		newLocation = cmnds[1]
	}
	if len(cmnds) < 2 {
	...
	if !places.IsRoom(newLocation) && !places.IsGlobalPlace(newLocation) {
		return "нет такого места"
	}
	if d.User.Location == newLocation {
		return "ты и так тут"
	}
	if !d.Places[d.User.Location].FindEnter(cmnds[1]) {
		return "нет пути в " + newLocation
	}
	if d.Places[newLocation].Lock.Locked {
		return d.Places[newLocation].Lock.Barrier +
			d.Places[newLocation].Lock.LockStatus
	}
	d.User.Location = newLocation
	var answer strings.Builder
	answer.WriteString(d.Places[newLocation].WelcomeText + "можно пройти - ")
	for i, plc := range d.Places[newLocation].Enter {
		answer.WriteString(plc)
		if i+1 < len(d.Places[newLocation].Enter) {
			answer.WriteString(", ")
		}
	}
